...
hmcsampler <- function(n_iter,                  # number of MCMC samples
  ...
  treePrec <- solve(beast$treeVcvRand)
  ...
  traitCovframe <- matrix(NA, ncol = P) # saving traitCov
  ...
  for (Iteration in 1:NumOfIterations) {
    ...
    if (Iteration > burnIn & Iteration %% thin == 0){
      ...
      write.table(as.data.frame(traitCovframe), file = "latent_covL8B50.csv", 
      ...
      chain_other <- data.frame(time = proc.time()[3] - timer[3], 
                                target = CurrentU, 
                                mdsPrecision = mdsPrec[Iteration])
      write.table(chain_other, file = "chain_otherL8B50.csv", append = TRUE,
                  row.names = FALSE, col.names = FALSE, sep = ',')

      locframe[1, ] <- as.vector(CurrentLocation)
      write.table(as.data.frame(locframe), file = "latent_coordL8B50.csv",
                  append = TRUE, row.names = FALSE, col.names = FALSE,
                  sep = ',')
    }
    ...
    ProposedMomentum = ProposedMomentum -
      StepSize/2 * Potential(engine = engine_lf, locations = ProposedLocation, 
                             #treePrecDet = treePrecDet,
                             treePrec = treePrec,
    ...
    for (StepNum in 1:NumOfLeapfrog) {
      ...
      if (StepNum != NumOfLeapfrog) {
        ...
        ProposedMomentum = ProposedMomentum -
          StepSize * Potential(engine = engine_lf, locations = ProposedLocation, 
                               #treePrecDet = treePrecDet,
                               treePrec = treePrec,
                               #traitVcv = solve(traitPrec[Iteration, , ]),
                               traitPrec = traitPrec[Iteration, , ],
    ...
    if (Iteration < n_iter) {
      if (Iteration %% mdsWeight == 0){
        ...
        # compare new and old sigma with current chain, not a symmetric proposal
